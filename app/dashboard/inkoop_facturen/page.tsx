...
export default function FacturenBeherenPage() {
    ...
    const handleDelete = async (id: string) => {
        ...
        const { error } = await supabase.from("inkoop_facturen").delete().eq("id", id);
        if (error) {
    ...
    const saveChanges = async () => {
        ...
        try {
            const { error } = await supabase
                .from('inkoop_facturen')
                .update({
                    naam_leverancier: editingData.naam_leverancier,
                                            ...
                                            <tr key={invoice.id} className={editingId === invoice.id ? "bg-blue-50 dark:bg-blue-900" : ""}>
                                                ...
                                                <td className="px-6 py-4 whitespace-nowrap text-sm">
                                                    {editingId === invoice.id ? (
                                                        <input
                                                            type="text"
                                                            value={editingData?.naam_leverancier || ""}
                                                            onChange={(e) => handleInputChange("naam_leverancier", e.target.value)}
                                                            className="w-full px-2 py-1 border rounded dark:bg-gray-700 dark:text-white"
                                                        />
                                                    ) : (
                                                        <span className="text-gray-900 dark:text-gray-100">{invoice.naam_leverancier}</span>
                                                    )}
                                                </td>
